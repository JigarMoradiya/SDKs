...
import com.jigar.me.databinding.RawLayoutSudokuPlay9by9Binding
...
class Play9By9Adapter(
...
) : RecyclerView.Adapter<Play9By9Adapter.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, i: Int): MyViewHolder {
        val view: View = LayoutInflater.from(viewGroup.context).inflate(
            R.layout.raw_layout_sudoku_play_9by9,
            viewGroup, false
        )
    ...
    inner class MyViewHolder(viewLayout: View?) :
        ...
        val binding: RawLayoutSudokuPlay9by9Binding = DataBindingUtil.bind(itemView)!!
    ...
    private fun SetValue(
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                ...
                if (temp_sudoku.isNotEmpty()) {
                    var setvalue = ""
                    if (temp_sudoku[0].cellValue.length > 0) {
                        try {
                            setvalue = (temp_sudoku[0].cellValue.toInt() + 1).toString() + ""
                        } catch (e: NumberFormatException) {
                            e.printStackTrace()
