...
import com.jigar.me.data.local.data.DataProvider.getGridList
import com.jigar.me.data.local.data.DataProvider.getRowList
...
import com.jigar.me.databinding.FragmentSudokuPlay9by9Binding
import com.jigar.me.ui.view.base.BaseFragment
import com.jigar.me.ui.view.confirm_alerts.dialogs.sudoku.SudokuCompleteDialog
import com.jigar.me.ui.view.dashboard.fragments.sudoku.play.adapter.Play9By9Adapter
...
@AndroidEntryPoint
class SudokuPlay9Fragment : BaseFragment(), Play9By9Adapter.ListClicks,
    SudokuCompleteDialog.SudokuCompleteDialogInterface {
    private val sudokuViewModel by viewModels<SudokuViewModel>()
    private lateinit var binding: FragmentSudokuPlay9by9Binding
    private lateinit var adapter: Play9By9Adapter
    private var roomId = ""
    private var level = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        level = SudokuPlay6FragmentArgs.fromBundle(requireArguments()).level
    }
    ...
    private fun initView() {
        ...
        list_grid.addAll(getGridList())
        list_column.addAll(getColumnList())
        list_row.addAll(getRowList())
        checkData()
        setData()
    }
    private fun initListener() {
        ...
        binding.txt4.onClick { setValueNumberClick("4") }
        binding.txt5.onClick { setValueNumberClick("5") }
        binding.txt6.onClick { setValueNumberClick("6") }
        binding.txt7.onClick { setValueNumberClick("7") }
        binding.txt8.onClick { setValueNumberClick("8") }
    ...
    private fun checkData() {
        ...
        lifecycleScope.launch {
            ...
            if (lsit_temp.isEmpty()) {
                ...
                if (sudokuViewModel.getRoomID_SudokuAnswer(roomId).isEmpty()){
    ...
    private fun setData() {
        ...
        lifecycleScope.launch {
            for (i in 0..8) {
                ...
                val isTrue = sudokuViewModel.getRoomID_CellValue(roomId, i.toString() + "").size == 9
        ...
        adapter = Play9By9Adapter(requireContext(), list_suduko, roomId,sudokuViewModel,prefManager,this)
    ...
    private fun buttonDisable(pre_value: String) {
        CoroutineScope(Dispatchers.Main).launch {
            if (pre_value.equals("0", ignoreCase = true)) {
                ...
                binding.txt4.setBackgroundResource(R.drawable.ractagle_gray500_border)
                binding.txt4.isEnabled = false
            } else if (pre_value.equals("5", ignoreCase = true)) {
                binding.txt5.setBackgroundResource(R.drawable.ractagle_gray500_border)
                binding.txt5.isEnabled = false
            } else if (pre_value.equals("6", ignoreCase = true)) {
                binding.txt6.setBackgroundResource(R.drawable.ractagle_gray500_border)
                binding.txt6.isEnabled = false
            } else if (pre_value.equals("7", ignoreCase = true)) {
                binding.txt7.setBackgroundResource(R.drawable.ractagle_gray500_border)
                binding.txt7.isEnabled = false
            } else if (pre_value.equals("8", ignoreCase = true)) {
                binding.txt8.setBackgroundResource(R.drawable.ractagle_gray500_border)
                binding.txt8.isEnabled = false
            }
        }
    }
    ...
    private fun buttonEnable(pre_value: String) {
        CoroutineScope(Dispatchers.Main).launch {
            if (pre_value.equals("0", ignoreCase = true)) {
                ...
                binding.txt4.setBackgroundResource(R.drawable.ractagle_white_border)
                binding.txt4.isEnabled = true
            } else if (pre_value.equals("5", ignoreCase = true)) {
                binding.txt5.setBackgroundResource(R.drawable.ractagle_white_border)
                binding.txt5.isEnabled = true
            } else if (pre_value.equals("6", ignoreCase = true)) {
                binding.txt6.setBackgroundResource(R.drawable.ractagle_white_border)
                binding.txt6.isEnabled = true
            } else if (pre_value.equals("7", ignoreCase = true)) {
                binding.txt7.setBackgroundResource(R.drawable.ractagle_white_border)
                binding.txt7.isEnabled = true
            } else if (pre_value.equals("8", ignoreCase = true)) {
                binding.txt8.setBackgroundResource(R.drawable.ractagle_white_border)
                binding.txt8.isEnabled = true
            }
        }
    }
    ...
    private fun setNote() {
        ...
        binding.txt4.setBackgroundResource(R.drawable.ractagle_notes_set_remain)
        binding.txt5.setBackgroundResource(R.drawable.ractagle_notes_set_remain)
        binding.txt6.setBackgroundResource(R.drawable.ractagle_notes_set_remain)
        binding.txt7.setBackgroundResource(R.drawable.ractagle_notes_set_remain)
        binding.txt8.setBackgroundResource(R.drawable.ractagle_notes_set_remain)
        lifecycleScope.launch {
            val notes: String =
    ...
    private fun notesSet() {
        ...
        lifecycleScope.launch {
            ...
            if (notes.isNotEmpty()) {
                ...
                binding.txt4.setBackgroundResource(R.drawable.ractagle_notes_set)
                binding.txt5.setBackgroundResource(R.drawable.ractagle_notes_set)
                binding.txt6.setBackgroundResource(R.drawable.ractagle_notes_set)
                binding.txt7.setBackgroundResource(R.drawable.ractagle_notes_set)
                binding.txt8.setBackgroundResource(R.drawable.ractagle_notes_set)
            }
        }
    ...
    private fun hintRemove() {
        lifecycleScope.launch {
            if (prefManager.getCustomParam(SudukoConst.Notes, "0") == "0") {
                ...
                for (i in 0..8) {
                    val isWrongAnswerNotThere = sudokuViewModel.getRoomID_SudokuAnswer_CheckCellValue(roomId, i.toString() + "").isEmpty()
                    val isTrue = sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, i.toString() + "").size == 9
...
//                binding.txtHint.setBackgroundResource(R.drawable.ractagle_hint)
                ...
                val temp_list_number: ArrayList<String> =
                    sudokuViewModel.getRoomID_CellPosition_IN_List_For_Remove_SudokuPlay(roomId, str_column+ "," + str_grid + "," + str_row, list_suduko1)
    ...
    private fun buttonHintShow(pre_value: String) {
        if (pre_value.equals("0", ignoreCase = true)) {
            ...
            binding.txt4.setBackgroundResource(R.drawable.ractagle_hint_set)
        } else if (pre_value.equals("5", ignoreCase = true)) {
            binding.txt5.setBackgroundResource(R.drawable.ractagle_hint_set)
        } else if (pre_value.equals("6", ignoreCase = true)) {
            binding.txt6.setBackgroundResource(R.drawable.ractagle_hint_set)
        } else if (pre_value.equals("7", ignoreCase = true)) {
            binding.txt7.setBackgroundResource(R.drawable.ractagle_hint_set)
        } else if (pre_value.equals("8", ignoreCase = true)) {
            binding.txt8.setBackgroundResource(R.drawable.ractagle_hint_set)
        }
    }
        ...
        lifecycleScope.launch {
            ...
            if (prefManager.getCustomParam(SudukoConst.SelectedBox, "").isNotEmpty()) {
                ...
                for (k in temp_sudoku.indices) {
                    ...
                    val lst_temp: List<String> =
                        sudokuViewModel.getRoomID_CellPosition_IN_Quate_SudokuPlay(
                            str_value + "", roomId,
                    ...
                    if (str_temp.contains(",")) {
                        ...
                        //                    Log.e("lst_temp1", "" + lst_temp1.size());
                        if (lst_temp1.size > 0) {
                    ...
                    if (iii.isNotEmpty()) {
                            if (sudokuViewModel.getRoomID_SudokuAnswer(roomId).isEmpty() && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "0").size == 9) && (
                                sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "1").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "2").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "3").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "4").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "5").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "6").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "7").size == 9
                                ) && (sudokuViewModel.getRoomID_CellValue_SudokuPlay(roomId, "8").size == 9)
                            ){
    ...
    private fun buttonNotesSet(pre_value: String) {
        if (pre_value.equals("1", ignoreCase = true)) {
            ...
            binding.txt7.setBackgroundResource(R.drawable.ractagle_notes_set)
        } else if (pre_value.equals("9", ignoreCase = true)) {
            binding.txt8.setBackgroundResource(R.drawable.ractagle_notes_set)
        }
    }
