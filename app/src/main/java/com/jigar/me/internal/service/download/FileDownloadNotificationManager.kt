 ...
 *
 * */
abstract class FileDownloadNotificationManager(context: Context) : FetchNotificationManager {
    ...
    override val broadcastReceiver: BroadcastReceiver
        get() = object: BroadcastReceiver() {
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                onDownloadNotificationActionTriggered(context, intent, this@FileDownloadNotificationManager)
            }
    ...
    override fun getNotificationBuilder(notificationId: Int, groupId: Int): NotificationCompat.Builder {
        synchronized(downloadNotificationsMap) {
            ...
            with(downloadNotificationsMap[notificationId]){
                if (this?.isCompleted == true){
                    val pdfFile = File(context.downloadFilePath()+"/"+ title)
                    val path: Uri
                    ...
                    pdfIntent = Intent(context , MainDashboardActivity::class.java)
                    pdfIntent.action = "android.intent.action.SEND"
                    pdfIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                }
            }
            ...
            notificationBuilder
                    ...
                    .setSmallIcon(android.R.drawable.stat_sys_download_done)
                    .mActions.clear()
//            logMessage("Notification Id by Notification Id $notificationId")
//            logMessage("Notification Id by Group Id $groupId")
