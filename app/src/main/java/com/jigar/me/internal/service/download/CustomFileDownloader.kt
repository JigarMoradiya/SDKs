...
import com.tonyodev.fetch2okhttp.getDefaultCookieJar
 ...
 * */
open class CustomFileDownloader @JvmOverloads constructor(
        /** OkHttpClient */
    ...
    private fun getResponseHeaders(okResponseHeaders: Headers): MutableMap<String, List<String>> {
        ...
        for (i in 0 until okResponseHeaders.size) {
            ...
            if (key != null) {
                ...
                headers[key.lowercase(Locale.getDefault())] = values
            }
        }
        return headers
    }
    ...
    private fun isReferrerNeeded(okHttpResponse: Response): Boolean = !okHttpResponse.body?.contentType().toString().isNullOrBlank() &&
        okHttpResponse.body?.contentType().toString().contains("text/html")
    ...
    override fun execute(request: Downloader.ServerRequest, interruptMonitor: InterruptMonitor): Downloader.Response? {
        ...
        } else if (isReferrerNeeded(okHttpResponse)) {
            if (okHttpRequest.header("Referer") == null) {
                val referer = getRefererFromUrl(request.url)
                okHttpRequest = okHttpRequest.newBuilder()
                    .addHeader("Referer", referer)
